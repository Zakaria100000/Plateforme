...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $stmt = $pdo->prepare("UPDATE annee SET nom_annee=? WHERE id=?");

    if(!$stmt->execute([ $nom_annee,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: annee-read.php?id=$id");
    }
} else {
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
                    ...
                    $nom_annee = htmlspecialchars($row["nom_annee"]);
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="annee-index.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
